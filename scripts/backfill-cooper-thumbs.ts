/**
 * Backfill product thumbnails for Cooper Lighting products by navigating to each product page
 * and screenshotting the hero image element.
 * Run with: npm run backfill:cooper-thumbs
 */
...
const THUMB_DIR = path.join(process.cwd(), 'public', 'thumbnails', 'cooper')
...
async function captureThumbnail(
  ...
  try {
    ...
    const imgEl = page.locator('img[src*="assets.cooperlighting.com/is/image/CLS"]').first()
...
async function main() {
  const cooper = await prisma.manufacturer.findUnique({ where: { slug: 'cooper' } })
  if (!cooper) { console.error('Cooper manufacturer not found'); process.exit(1) }

  const all = await prisma.product.findMany({
    where: { manufacturerId: cooper.id, productPageUrl: { not: null } },
    select: { catalogNumber: true, productPageUrl: true },
  })

  const products = all.filter(p => !fs.existsSync(getThumbPath(p.catalogNumber)))
  console.log(`Found ${products.length} Cooper products needing thumbnails (${all.length - products.length} already cached)`)
