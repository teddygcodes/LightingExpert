/**
 * Backfill product thumbnails for Acuity products by navigating to each product page
 * and screenshotting the hero image element (bypasses Cloudflare CDN restrictions).
 * Run with: npx ts-node --project tsconfig.scripts.json scripts/backfill-acuity-thumbs.ts
 */
...
const THUMB_DIR = path.join(process.cwd(), 'public', 'thumbnails', 'acuity')
const CONTEXT_RECYCLE_INTERVAL = 30

function getThumbPath(catalogNumber: string): string {
  return path.join(THUMB_DIR, `${catalogNumber}.png`)
}

function getThumbPublicPath(catalogNumber: string): string {
  return `/thumbnails/acuity/${catalogNumber}.png`
}
...
async function captureThumbnail(
  ...
  try {
    ...
    const imgEl = page.locator('img[src*="img.acuitybrands.com/public-assets/catalog/"]').first()
...
async function main() {
  ...
  // Find products that need thumbnails (have a product page URL, no cached thumbnail)
  const all = await prisma.product.findMany({
    where: { manufacturerId: acuity.id, productPageUrl: { not: null } },
    select: { catalogNumber: true, productPageUrl: true },
  })

  const products = all.filter(p => !fs.existsSync(getThumbPath(p.catalogNumber)))
  console.log(`Found ${products.length} Acuity products needing thumbnails (${all.length - products.length} already cached)`)
